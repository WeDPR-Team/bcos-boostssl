...
public:
    ...
    void handleRequest(HttpRequest&& _httpRequest)
    {
        ...
        auto self = std::weak_ptr<HttpSession>(shared_from_this());
        if (m_httpUrlReqHandler && _httpRequest["has_uri"] == std::to_string(true))
        {
            m_httpUrlReqHandler(_httpRequest, [self, version, startT](bcos::bytes _content) {
                ...
                BCOS_LOG(TRACE) << LOG_BADGE(session->m_moduleName) << LOG_BADGE("handleUrlRequest")
                                << LOG_DESC("response")
    ...
    HttpUrlReqHandler httpUrlReqHandler() const { return m_httpUrlReqHandler; }
    void setHttpUrlReqHandler(HttpUrlReqHandler _httpUrlReqHandler)
    {
        m_httpUrlReqHandler = _httpUrlReqHandler;
    }
...
private:
    ...
    HttpUrlReqHandler m_httpUrlReqHandler;
